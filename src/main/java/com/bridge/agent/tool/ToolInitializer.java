...
/**
 * 工具注册器 —— 应用启动时将所有工具注册到 ToolRegistry
 *
 * <p>面试要点：
 * "为什么手动注册而不用 Spring AI 的 @Tool 注解自动扫描？
 *  → @Tool 注解配合 Spring AI ChatClient 的自动 function calling，
 *    框架控制循环，我们无法在中间插入质量校验、轨迹记录等自定义逻辑。
 *  → 手动注册到 ToolRegistry，配合自定义 ReAct Engine，
 *    每个工具调用都经过我们的代码，可控、可观测、可调试。"
 */
@Component
public class ToolInitializer {
    ...
    public void initTools() {
        ...
        registry.register(
                ...
                "查询指定桥梁在指定时间范围内的历史病害记录列表（按时间倒序）。" +
                "适用：检修前了解历史病害情况，或追溯某类病害的发展历史时。",
                "{\"bridgeId\": \"桥梁编号\", \"months\": \"查询最近几个月，默认36\"}",
                defectHistoryTool::execute
        );

        registry.register(
                "query_repair_records",
                "查询指定桥梁的历史维修和加固记录（按时间倒序）。" +
                "适用：检修前了解近期维修情况，评估构件维修后的状态时。",
                "{\"bridgeId\": \"桥梁编号\"}",
        ...
        registry.register(
                ...
                "适用：检修前分析哪类病害最频繁出现，需要重点关注时。" +
                "不适用：查询单次检测的病害详情。",
                "{\"bridgeId\": \"桥梁编号\", \"months\": \"分析最近几个月，默认36\"}",
        ...
        registry.register(
                "normalize_defect",
                "将检测员的自由文本病害描述转化为符合 JTG/T H21 规范格式的标准化记录，" +
                "包含规范化描述、病害等级、规范条款引用和定级依据。" +
                "适用：信息已足够完整，需要生成正式的病害记录时。" +
                "不适用：信息仍不完整时（应先通过 ASK_USER 追问）。",
        ...
        registry.register(
                "search_repair_cases",
                "检索历史同类病害的处置方案（修复工艺、处置时限等），" +
                "为本次病害处置提供参考案例。" +
                "适用：检修后生成处置建议时，需要参考历史做法时。",
                "{\"defectType\": \"病害类型\", \"bridgeType\": \"桥型（可选）\", \"grade\": \"病害等级（可选）\"}",
                repairCaseTool::execute
        );

        log.info("Tool registry initialized with {} tools", 7);
    }
}
